...
if Modules == nil then
    ...
    function StdModule.promotePlayer(cid, message, keywords, parameters, node)
        ...
        if npcHandler == nil then
            error(
                "StdModule.promotePlayer called without any npcHandler instance.")
        end
        ...
        if player:isPremium() or not parameters.premium then
            ...
            npcHandler:say(
                "You need a premium account in order to get promoted.", cid)
        end
        npcHandler:resetNpc(cid)
        return true
    end
    ...
    function StdModule.learnSpell(cid, message, keywords, parameters, node)
        ...
        if player:isPremium() or not parameters.premium then
            ...
            elseif not player:removeTotalMoney(parameters.price) then
                npcHandler:say(
                    "You do not have enough money, this spell costs " ..
                        parameters.price .. " gold.", cid)
            else
            ...
            npcHandler:say("You need a premium account in order to buy " ..
                               parameters.spellName .. ".", cid)
        end
        npcHandler:resetNpc(cid)
        return true
    end
    ...
    function StdModule.bless(cid, message, keywords, parameters, node)
        ...
        if not npcHandler:isFocused(cid) or Game.getWorldType() ==
            WORLD_TYPE_PVP_ENFORCED then return false end

        local player = Player(cid)
        if player:isPremium() or not parameters.premium then
            if player:hasBlessing(parameters.bless) then
                npcHandler:say(
                    "Gods have already blessed you with this blessing!", cid)
            elseif not player:removeTotalMoney(parameters.cost) then
    ...
    function StdModule.travel(cid, message, keywords, parameters, node)
        ...
        if player:isPremium() or not parameters.premium then
            if player:isPzLocked() then
                npcHandler:say(
                    "First get rid of those blood stains! You are not going to ruin my vehicle!",
                    cid)
            elseif parameters.level and player:getLevel() < parameters.level then
                npcHandler:say("You must reach level " .. parameters.level ..
                                   " before I can let you go there.", cid)
            elseif not player:removeTotalMoney(parameters.cost) then
            ...
            npcHandler:say(
                "I'm sorry, but you need a premium account in order to travel onboard our ships.",
                cid)
        end
        npcHandler:resetNpc(cid)
        return true
    ...
    function FocusModule:init(handler)
        ...
        for i, word in pairs(FOCUS_GREETWORDS) do
            local obj = {}
            obj[#obj + 1] = word
            obj.callback = FOCUS_GREETWORDS.callback or
                               FocusModule.messageMatcher
            handler.keywordHandler:addKeyword(obj, FocusModule.onGreet,
                                              {module = self})
        end
        ...
        for i, word in pairs(FOCUS_FAREWELLWORDS) do
            local obj = {}
            obj[#obj + 1] = word
            obj.callback = FOCUS_FAREWELLWORDS.callback or
                               FocusModule.messageMatcher
            handler.keywordHandler:addKeyword(obj, FocusModule.onFarewell,
                                              {module = self})
        end
    ...
    function KeywordModule:parseParameters()
        ...
        if ret then self:parseKeywords(ret) end
    end
    ...
    function TravelModule:init(handler)
        self.npcHandler = handler
        self.yesNode = KeywordNode:new(SHOP_YESWORD, TravelModule.onConfirm,
                                       {module = self})
        self.noNode = KeywordNode:new(SHOP_NOWORD, TravelModule.onDecline,
                                      {module = self})
        self.destinations = {}
        return true
    end
    ...
    function TravelModule:parseParameters()
        ...
        if ret then
            ...
            self.npcHandler.keywordHandler:addKeyword({"destination"},
                                                      TravelModule.listDestinations,
                                                      {module = self})
            self.npcHandler.keywordHandler:addKeyword({"where"},
                                                      TravelModule.listDestinations,
                                                      {module = self})
            self.npcHandler.keywordHandler:addKeyword({"travel"},
                                                      TravelModule.listDestinations,
                                                      {module = self})
    ...
    function TravelModule:addDestination(name, position, price, premium)
        ...
        local node = self.npcHandler.keywordHandler:addKeyword(keywords,
                                                               TravelModule.travel,
                                                               parameters)
        self.npcHandler.keywordHandler:addKeyword(keywords2,
                                                  TravelModule.bringMeTo,
                                                  parameters)
        ...
        if npcs_loaded_travel[getNpcCid()] == nil then
            ...
            self.npcHandler.keywordHandler:addKeyword({'yes'},
                                                      TravelModule.onConfirm,
                                                      {module = self})
            self.npcHandler.keywordHandler:addKeyword({'no'},
                                                      TravelModule.onDecline,
                                                      {module = self})
        end
    end
    ...
    function TravelModule.travel(cid, message, keywords, parameters, node)
        ...
        module.npcHandler:say("Do you want to travel to " .. keywords[1] ..
                                  " for " .. cost .. " gold coins?", cid)
        return true
    end
    ...
    function TravelModule.onConfirm(cid, message, keywords, parameters, node)
        ...
        if shop_npcuid[cid] ~= Npc().uid then return false end
    ...
    function TravelModule.listDestinations(cid, message, keywords, parameters,
                                           node)
        local module = parameters.module
        if not module.npcHandler:isFocused(cid) then return false end
    ...
    function ShopModule:parseParameters()
        ...
        if ret then self:parseBuyable(ret) end

        local ret = NpcSystem.getParameter("shop_sellable")
        if ret then self:parseSellable(ret) end

        local ret = NpcSystem.getParameter("shop_buyable_containers")
        if ret then self:parseBuyableContainers(ret) end
    end
            ...
            if SHOPMODULE_MODE == SHOPMODULE_MODE_TRADE then
                ...
                if name and itemid and cost then
                    if subType == nil and it:isFluidContainer() then
                        ...
                        local names = {}
                        names[#names + 1] = name
                        self:addBuyableItem(names, itemid, cost, subType,
                                            realName)
                    end
                else
                    print("[Warning : " .. Npc():getName() .. "] NpcSystem:",
        ...
        for item in string.gmatch(data, "[^;]+") do
            ...
            if name and container and itemid and cost then
                if subType == nil and ItemType(itemid):isFluidContainer() then
                    ...
                    local names = {}
                    names[#names + 1] = name
                    self:addBuyableItemContainer(names, container, itemid, cost,
                                                 subType, realName)
                end
            else
                print("[Warning : " .. Npc():getName() .. "] NpcSystem:",
    ...
    function ShopModule:init(handler)
        self.npcHandler = handler
        self.yesNode = KeywordNode:new(SHOP_YESWORD, ShopModule.onConfirm,
                                       {module = self})
        self.noNode = KeywordNode:new(SHOP_NOWORD, ShopModule.onDecline,
                                      {module = self})
        ...
        if SHOPMODULE_MODE ~= SHOPMODULE_MODE_TALK then
            for i, word in pairs(SHOP_TRADEREQUEST) do
                local obj = {}
                obj[#obj + 1] = word
                obj.callback = SHOP_TRADEREQUEST.callback or
                                   ShopModule.messageMatcher
                handler.keywordHandler:addKeyword(obj, ShopModule.requestTrade,
                                                  {module = self})
            end
        end
    ...
    function ShopModule:reset() self.amount = 0 end

    -- Function used to match a number value from a string.
    function ShopModule:getCount(message)
        local ret = 1
        local b, e = string.find(message, PATTERN_COUNT)
        if b and e then ret = tonumber(string.sub(message, b, e)) end
    ...
    function ShopModule:addBuyableItem(names, itemid, cost, itemSubType,
                                       realName)
        if SHOPMODULE_MODE ~= SHOPMODULE_MODE_TALK then
        ...
        if npcs_loaded_shop[getNpcCid()] == nil then
            ...
            self.npcHandler.keywordHandler:addKeyword({'yes'},
                                                      ShopModule.onConfirm,
                                                      {module = self})
            self.npcHandler.keywordHandler:addKeyword({'no'},
                                                      ShopModule.onDecline,
                                                      {module = self})
        end
    end
    ...
    function ShopModule:getShopItem(itemId, itemSubType)
        if ItemType(itemId):isFluidContainer() then
            ...
            for i = 1, #self.npcHandler.shopItems do
                local shopItem = self.npcHandler.shopItems[i]
                if shopItem.id == itemId then return shopItem end
            end
        end
        return nil
    end
    ...
    function ShopModule:callbackOnBuy(cid, itemid, subType, amount, ignoreCap,
        ...
        if inBackpacks then
            totalCost = ItemType(itemid):isStackable() and totalCost + 20 or
                            totalCost +
                            (math.max(1, math.floor(
                                          amount /
                                              ItemType(ITEM_BACKPACK):getCapacity())) *
                                20)
        end
    ...
    function ShopModule.requestTrade(cid, message, keywords, parameters, node)
        ...
        if not module.npcHandler:onTradeRequest(cid) then return false end
        ...
        if itemWindow[1] == nil then
            local parseInfo = {[TAG_PLAYERNAME] = Player(cid):getName()}
            local msg = module.npcHandler:parseMessage(
                            module.npcHandler:getMessage(MESSAGE_NOSHOP),
                            parseInfo)
            module.npcHandler:say(msg, cid)
            return true
        end
        ...
        openShopWindow(cid, itemWindow,
                       function(cid, itemid, subType, amount, ignoreCap,
                                inBackpacks)
            module.npcHandler:onBuy(cid, itemid, subType, amount, ignoreCap,
                                    inBackpacks)
        end, function(cid, itemid, subType, amount, ignoreCap, inBackpacks)
            module.npcHandler:onSell(cid, itemid, subType, amount, ignoreCap,
                                     inBackpacks)
        end)
        module.npcHandler:say(msg, cid)
        return true
    ...
    function ShopModule.onConfirm(cid, message, keywords, parameters, node)
        ...
        if shop_eventtype[cid] == SHOPMODULE_SELL_ITEM then
            local ret = doPlayerSellItem(cid, shop_itemid[cid],
                                         shop_amount[cid],
                                         shop_cost[cid] * shop_amount[cid])
            if ret == true then
    ...
    function ShopModule.tradeItem(cid, message, keywords, parameters, node)
        ...
        if not module.npcHandler:onTradeRequest(cid) then return true end
    ...
    function VoiceModule:init(handler) return true end
