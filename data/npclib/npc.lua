...
local sayFunction = function(npcId, text, type, eventDelay, playerId)
	local npc = Npc(npcId)
	if not npc then
		logError("[sayFunction] - Npc not is valid")
...
-- BUG FIX (Crystal Server): original triple-condition was logically impossible.
-- Fixed to: simple case-insensitive substring search.
function MsgFind(message, keyword)
	local lowerMessage, lowerKeyword = message:lower(), keyword:lower()
	if lowerMessage == lowerKeyword then
		return true
	end
	return string.find(lowerMessage, lowerKeyword) ~= nil
end
...
-- BUG FIX (Crystal Server): original called npc:getName() before nil check.
function SayEvent(npcId, playerId, messageDelayed, npcHandler, textType)
	local npc = Npc(npcId)
	if not npc then
		logError("[SayEvent] - Npc not found for id: " .. tostring(npcId))
		return
	end

	local player = Player(playerId)
	if not player then
		logError("[SayEvent] - Player not found for id: " .. tostring(playerId))
		return
	end
...
function GetCount(str)
	...
	return b and e and tonumber(str:sub(b, e)) or -1
end
